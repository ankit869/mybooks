        ...
        if (req.isAuthenticated()) {
            ...
            admin_promise.then((result) => {
                if (isAdmin && isUnicode) {
                    res.render("admin/admin_success", { status: "none", message: "Thanks " + req.user.name + " for your contribution !" })
                } else {
            ...
            admin_promise.then((result) => {
                ...
                if (isAdmin && isUnicode) {
                    ...
                    if(req.params.book_id=="newbook"){
                        res.render("admin/upload_edit", { status: "none", book: '', isupt: isupt })
                    }else{
                        BOOK.findOne({ _id: req.params.book_id }, (err, book) => {
                            if(err){
                                ...
                                if (book) {
                                    res.render("admin/upload_edit", { status: "none", book: book, isupt: isupt })
                                }else{
                                    res.redirect("/error")
                                }
                            }
...
router.get('/admin/add_to_review/:book_id/:book_name',async (req, res) => {
                    ...
                    contact_promise.then((result) => {
                        ...
                        CONTACT.findOne({ _id: req.body.client_mailId }, (err, contact) => {
                            if (!err) {
                                ...
                                USER.findOneAndUpdate({ username: req.user.username }, { $inc: { "account_record.q_resolved": 1 } }, { new: true }, (err, user) => { if (err) {  } })
                                USER.findOneAndUpdate({ username: req.user.username }, { $inc: { "account_record.credits": 1.5 } }, { new: true }, (err, user) => { if (err) {  } })
                            }
                        })
            ...
            admin_promise.then((result) => {
                if (isAdmin && isUnicode) {
                    ...
                    user_promise.then((result) => {
                        USER.updateOne({ _id: req.params.id }, { $set: { ismember: true, membershipType:"naive_user",searchtag: searchtag + "-member-naive_user" } }, (err) => {
                            if (!err) {
                    ...
                    BOOK.findOne({ _id: req.params.book_id }, async(err, book) => {
                        if(err){
                            ...
                            if(book){
                                ...
                                deleted_book = book
                                deleted_book.deletedAt=Date.now();
                                deleted_book.save();
                                book.remove();
                    ...
                    if (searchtag == "allusers" || searchtag == "") {
                        ...
                        USER.find({}).limit(5).exec((err, members) => {
                            if(err){
                                ...
                                memberPromise.then(()=>{
                                    res.render('admin/members_detail', { status: "none", members: usr_details, currentAdmin: req.user.username, searchtag: "allusers" })
                                })
                        ...
                        USER.find({ searchtag: { $regex: searchtag, $options: '$i' } }).limit(5).exec((err, members) => {
                            if(err){
                                ...
                                memberPromise.then(()=>{
                                    res.render('admin/members_detail', { status: "none", members: usr_details, currentAdmin: req.user.username, searchtag: searchtag })
                                })
        ...
        if (req.isAuthenticated()) {
            ...
            const signupPromise = new Promise((resolve, reject) => {
                ADMINUSER.findOne({ username: req.body.username }, (err, admin) => {
                    if (admin) {
                        res.send({ message: "Error: This user is already being registered" })
                    } else {
                        resolve(true)
                    }
                })
...
router.post('/admin-login', async (req, res) => {
    try {
        if(req.isAuthenticated()){
            ...
            admin_promise.then(async (result) => {
                if (isAdmin && isUnicode) {
                    res.redirect("/home")
                }else{
                    ...
                    if (req.body.adminkey == process.env.ADMIN_KEY) {
                        if (await validateOTP(req.body.username, req.body.otp) || otp.validateOTP(req.body.username, OTP)) {
                            ADMINUSER.findOne({ userId: req.user.id }, (err, admin) => {
                                if(err){
                                    log(error, path.join(__dirname,'../error.log'))
                                }else{
                                    if (admin) {
                                        if (admin.unicode == req.body.unicode) {
